...
* Die Profile haben den Vorteil verschieden Konfigurationen bei unterschiedlichen Anwendungfällen zur Verfügung zu stellen. ZB wir eine eigen Datenbank zum testen verwendet
* man erkennt das jeweilige Profile in der `application.properties` durch den Prefix %prod, %test, %dev
...
** meist wird eine eigen Testdatenbank gestartet
* prod
** Ist optimierter (zB startet schneller)
